...
(function($) {
	...
	function stickToTop(name, fullWidth, options) {
		...
		function getMenuPosition() {
			menu.addClass('stick-to-top-menu');
			if(!menu.hasClass('stick')) { 
				menu.data("top", menu.offset().top);
				menu.data("originalWidth", menu.outerWidth(false));
				menu.data('originalLeft',  menu.offset().left);
			} else {
				menu.removeClass('stick').css('left', '0px');
				menu.data("originalWidth", menu.outerWidth(false));
				menu.data('originalLeft',  menu.offset().left);
				menu.css({
	...
	function alignMenu(name, options) { 
		...
		//console.log('align menu ' + $(container).outerWidth());

		switch(align) { 
			case "center" : 
				var containerWidth;
				if($(container).data("originalAlignWidth")) { 
					containerWidth =  $(container).data("originalAlignWidth");	
				} else { 
					containerWidth = $(container).outerWidth();
					$(container).data("originalAlignWidth", $(container).outerWidth());
				}
...
(function($) {
	...
	function xtdCSSMenuInit(Name, settings)
	{
...
function registerFlexiCSSMenu(name, settings) {
	...
	if(!flexiCssMenus.xtdCSSMenuInit(name, settings)) {
...
(function($) {
	...
	function doneResizing() {
		...
		for(var i=0; i < registeredFlexiMenus.length; i++) { 
			...
			if(!flexiCssMenus.xtdCSSMenuInit(menu.name, menu.settings)) {
