...
(function(global, factory) {
    ...
    var cpIsMobileDevice = DeviceDetect.isMobile(viewportWidth, 768);
 
    function getScriptFolder() {
      var scripts = document.getElementsByTagName('SCRIPT');
      for (var i = 0; i < scripts.length; i++) {
        var src = scripts[i].getAttribute('src');
        if (src && src.indexOf('jquery.fullpage.js') != -1) {
          if (src.lastIndexOf('includes/') != -1) {
            return src.substring(0, src.lastIndexOf('includes/'));
          } else {
            return '';
          }
        }
    ...
    jQuery.fn.DWFullPageScreen = function(dwoptions) {
        var header, alternateHeder, pageContent, sections, footer, data,
            AUTO_HEIGHT = 'fp-auto-height',
            stickyHeader = '.alternate-header',
            isAfterPageLoadedSwitch = true;
        ...
        function prepareHeader(options) {
            $('body > .header, body > .alternate-header').css('position', 'fixed');
            header = $('body > .header');
              if (!options.showHeader) {
                ...
                options.headerHeight = header.outerHeight();

                var stickyHeaderEls = header.siblings(stickyHeader)
                if (stickyHeaderEls.length) {
                    alternateHeder = stickyHeaderEls;
                    alternateHeder.attr('data-managed', true);
                    toStyle = toStyle.add(alternateHeder);
                }
      ...
      function addBackgroundOverlayCompatibility() {
          var insertRule = function(selector, rules) {
          ...
          if (navigator.userAgent.toLowerCase().indexOf('edge/') !== -1 || ("ActiveXObject" in window)) {
              insertRule(['.fp-section::before', '.pp-section::before'], 'display:table-cell!important;z-index:1!important');
              insertRule(['.fp-section .fp-tableCell > *:not(.vidbg-container):not(.backstretch)', '.pp-section .pp-tableCell > *:not(.vidbg-container):not(.backstretch)'], 'z-index:2!important; position:relative');
          } else if (navigator.userAgent.toLowerCase().indexOf('firefox/') !== -1) {
              insertRule(['.fp-section::before', '.pp-section::before'], 'display:table-cell!important;');
          } else if (cpIsMobileDevice || navigator.userAgent.toLowerCase().indexOf('safari/') !== -1) {
              insertRule(['.fp-section::before, .pp-section::before'], 'transform: translate3d(0,0,0)!important;height:auto!important;min-height:100%!important;display:block!important;position: absolute;');
          }
        ...
        function addSectionCompatibility(index, currentSection, winY) {

            // revealFX
            if (!currentSection.data('revealFXContentTriggered')) {
                var revealItems = currentSection.find('[reveal-fx],[data-scrollreveal]');
                if (currentSection.is('[reveal-fx]')) {
                    revealItems.add(currentSection);
                }
                revealItems.removeAttr('data-scrollreveal-complete');
                revealItems.removeAttr('data-scrollreveal-initialized');
                revealItems.trigger('cp-revealfx-init');
                currentSection.data('revealFXContentTriggered', true);
            }

            if (options.autoScrolling && $(document).data().cpShrinkOnScroll) {
                if (index !== 1) {
                    $(document).data().cpShrinkOnScroll(winY);
                } else {
                    $(document).data().cpShrinkOnScroll(0);
                }
            }
        }

        function switchHeadersIfNeeded(nextSection, index, nextIndex) {
            var useAlternate = !!(nextSection.attr('data-alternate-header') && alternateHeder);
            var duration = isAfterPageLoadedSwitch ? 0 : options.scrollingSpeed;


            if (nextSection.is('.' + AUTO_HEIGHT) && index && nextIndex) {
            ...
            if (useAlternate) {
                header.fadeOut(fadeOptions);
                alternateHeder.fadeIn(fadeOptions);
            } else {
                header.fadeIn(fadeOptions);
                if (alternateHeder) {
                    alternateHeder.fadeOut(fadeOptions);
                }
            }
        }

        function changeMenuActiveElement() {
            function changeOneLevelMenus() {
                var oneLevelMenus = $('.header ul, .alternate-header ul');
                oneLevelMenus.each(function(index, el) {
                    $(el).find('.sel').removeClass('sel');
                    var href = (window.location + "");

                    var newSelected = $(el).find('a').filter(function(index, link) {
                        return (link.href == href || link.href == href.replace(/(&|\?)?preview_[0-9]+(=1)?/, ''));
                    });

                    var optionToSelect = $(el).find('select').children().filter(function(index, link) {
                        return (link.value == href || link.value == href.replace(/(&|\?)?preview_[0-9]+(=1)?/, ''));
                    }).attr('value');


                    $(el).find('select').val(optionToSelect);

                    newSelected.addClass('sel');

                });
            }

            setTimeout(changeOneLevelMenus, 0);
        }
        ...
        function getY() {
            ...
            if (options.scrollBar) {
                ...
                result = parseFloat(($el.css('top').match(/([0-9]+)/ig) || [0]).pop())
            }

            return Math.abs(result);
        }

        var arrow = jQuery('.fp-arrow');

        function onLeave(index, nextIndex, section) {
           jQuery('.fp-controlArrow.fp-prev').addClass('fa fa-chevron-left');
           jQuery('.fp-controlArrow.fp-next').addClass('fa fa-chevron-right');
            
            var sections = section.parent().children('.section');
            switchHeadersIfNeeded(sections.eq(nextIndex - 1), index, nextIndex);
            updateArrowColor(sections.eq(nextIndex - 1));

            if (nextIndex == section.siblings('.section').length + 1) {
                arrow.hide();
            } else {
                arrow.show();
            }
        ...
        function onLoad(index, section) {
            var winY = getY();
            addSectionCompatibility(index, section, winY);
            changeMenuActiveElement();
        }

        function onRender(activeSection) {
            if (isAfterPageLoadedSwitch) {
                switchHeadersIfNeeded(activeSection);
                isAfterPageLoadedSwitch = false;
            }
        }

        function prepareVideos(options) {
            var path = getScriptFolder();
            jQuery('[data-video]').each(function() {
                try {
                    var $this = jQuery(this);
                    var id = $this.attr('data-video');
                    if (options.videos[id]) {
                        var settings = options.videos[id];
                        $this.vidbg({
                            'mp4': path + settings.mp4,
                            'webm': path + settings.webm,
                            'poster': path + settings.poster,
                        }, {
                          id:     id,
     ...
     function updateArrowColor(section) {
          var color = "";

          if (section.attr('data-arrow') && section.attr('data-arrow') !== 'inherit') {
              color = section.attr('data-arrow');
          }

          if (arrow) {
        arrow.find('.arrow').css({
          'color': color,
        });
          }
      }
      ...
      addBackgroundOverlayCompatibility();


        if (cpIsMobileDevice) {
          dwoptions.bulletstype = "none";
          dwoptions.scrollBar = true;
          dwoptions.fitToSection = false;
          dwoptions.showArrow = false;
        }
        ...
        var options = jQuery.extend({
            ...
            scrollOverflow: !dwoptions.scrollBar,
            autoScrolling: !dwoptions.scrollBar,
            css3: false,

            onLeave: function(index, nextIndex) {
                onLeave(index, nextIndex, $(this))
            },
            afterLoad: function(ev, index) {
                onLoad(index, $(this))
            },
            afterRender: function() {
                onRender(this.children('.active'));
            }
        }, dwoptions);


        options.scrollingSpeed = parseInt(options.scrollingSpeed);
        
        prepareHeader(options);
        prepareFooter(options);
        prepareVideos(options);


        if (dwoptions.showArrow) {
          jQuery('.fp-arrow-c').show();
              } else {
          jQuery('.fp-arrow-c').hide();
        }

        if (dwoptions.arrowType) {
            jQuery('.fp-arrow .arrow.fa').attr('class', 'fa arrow ' + dwoptions.arrowType);
        }
        
         jQuery('.fp-arrow').click(function(){
            jQuery.fn.fullpage.moveSectionDown();
        });

         if (dwoptions.arrowBounce) {
            jQuery('.fp-arrow .arrow').addClass('bounce');
         } else {
            jQuery('.fp-arrow .arrow').removeClass('bounce');
         }

      
        if (!dwoptions.showFooter) {
            jQuery('.footer').remove();
        }

        switch (dwoptions.bulletstype) {
            case "onleft" :
                options.navigationPosition = "left"
                break;
            case "none" : 
        ...
        if (dwoptions.tooltiptype == "always") {
            options.showActiveTooltip = true;
        }



       
        jQuery(this).fullpage(options);
    }
...
!(function(root, factory) {
    ...
    Vidbg.prototype.init = function() {
        ...
        var wrapper_id = settings.id ? settings.id + "_wrapper" : "";
        $wrapper = vidbg.$wrapper = $('<div id="' + wrapper_id + '" class="vidbg-container">').css({
            position: 'absolute',
            'z-index': -2,
            top: 0,
            left: 0,
        ...
        var video_id = settings.id ? settings.id + "_video" : "";
